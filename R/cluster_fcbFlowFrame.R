...
cluster_fcbFlowFrame <- function(fcbFlowFrame,
                                 channel,
                                 levels,
                                 opt = "mixture",
                                 dist = NULL,
    ...
    if (opt_selected == "mixture") {
        if (levels > 1) {
            ...
            mod.int <- classInt::classIntervals(
                vecss, levels, style = "fisher"
            )
            classif <- findInterval(vecss, mod.int$brks)
            classif <- levels + 1 - classif
            mu.i <- as.numeric(
                unlist(lapply(split(vecss, classif), median))[-1]
            )
        }
        if (is.function(updateProgress)) {
            updateProgress(detail = "Optimizing Mixture Model")
        }
        Snorm.analysis <- mixsmsn::smsn.mix(vecss,
            nu = 3,
            g = levels,
            shape = rep(0, levels),
            mu = mu.i,
            get.init = TRUE, group = TRUE,
            family = dist_selected, calc.im = FALSE,
            kmeans.param = list(
                iter.max = 20, n.start = 10,
                algorithm = "Hartigan-Wong"
            )
        ) # sigma 2 parameter = mu.i
        ...
        probs.y <- apply(
            Snorm.df, 1,
            function(i) sn::dsn(vec, dp = as.numeric(i))
        )
