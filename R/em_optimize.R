...
em_optimize <- function(fcbFlowFrame,
    ...
    assert_fcbFlowFrame(
        fcbFlowFrame,
        needs = c("deskewing", "clustering", "assignment")
    )
    if (verbose) message("Initializing...")

    bc_names <- names(fcbFlowFrame@barcodes)
    barcodes <- fcbFlowFrame@barcodes[bc_names != "wells"]
    #  bc_initial.df <- as.data.frame(lapply(lapply(barcodes,`[[`, "assignment"), `[[`, "values"))
    # assigned.ind <- which(!apply(bc_initial.df == 0, 1 ,any))
    deskewed_cols <- as.data.frame(
        lapply(lapply(barcodes, `[[`, "deskewing"), `[[`, "values")
    )
    deskewed_models <- lapply(
        lapply(barcodes, `[[`, "clustering"), `[[`, "model"
    )
    deskewed_probs <- lapply(
        lapply(
            lapply(barcodes, `[[`, "clustering"),
            `[[`, "probabilities"
        ),
        as.matrix
    )
    ...
    deskewed.rep.mat <- kronecker(
        deskewed.dims.vec, t(rep(1, length(deskewed.dims.vec)))
    )
    ...
    deskewed.reps <- split(
        cbind(
            apply(deskewed.rep.mat.each, 2, prod),
    ...
    if (shrinkage > 0) {
        shrinkage.list <- lapply(deskewed_models, function(mod) {
            mixsmsn::rmix(
                ...
                family = class(mod),
                arg = split(
                    data.frame(mod[c("mu", "sigma2", "shape", "nu")]),
                    seq(length(mod$pii))
                ),
                cluster = TRUE
            )
        ...
        shrinkage.cl.mat <- mclust::unmap(
            as.factor(apply(shrinkage.cl, 1, paste0, collapse = "."))
        )
    ...
    # Component densities
    mvgmm.cdens <- esEst$parameters$pro * cdens(
        data = deskewed_cols,
        modelName = esEst$modelName[1],
    ...
    probs <- mvgmm.cdens / sum(mvgmm.tdens)
    ...
    wells_clust <- list(
        probabilities = probs,
        channels = deskewed.dims,
        model = esEst
    )
    fcbFlowFrame@barcodes[["wells"]][["clustering"]] <- wells_clust
